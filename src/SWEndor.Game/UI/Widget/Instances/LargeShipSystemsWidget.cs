            ...
            case RadarType.RECTANGLE_GIANT:
              {
                ...
                float range = (box.Z.Max - box.Z.Min) * scale;

                TVScreen2DImmediate.Draw_Box(box.X.Min * scale * radar_radius / range + radar_center.x
                                           , box.Z.Min * scale * radar_radius / range + radar_center.y
                                           , box.X.Max * scale * radar_radius / range + radar_center.x
                                           , box.Z.Max * scale * radar_radius / range + radar_center.y
                                           , icolor);
                break;
              }
            case RadarType.TRIANGLE_GIANT:
              {
                ...
                TVScreen2DImmediate.Draw_Triangle(box.X.Min * scale * radar_radius / range + radar_center.x
                                                , -box.Z.Min * scale * radar_radius / range + radar_center.y
                                                , (box.X.Min + box.X.Max) / 2 * scale * radar_radius / range + radar_center.x
                                                , -box.Z.Max * scale * radar_radius / range + radar_center.y
                                                , box.X.Max * scale * radar_radius / range + radar_center.x
                                                , -box.Z.Min * scale * radar_radius / range + radar_center.y
                                                , icolor);
