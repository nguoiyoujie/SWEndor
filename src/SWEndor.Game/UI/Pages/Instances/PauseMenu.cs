...
namespace SWEndor.Game.UI.Menu.Pages
{
  public class PauseMenu : Page
  {
    ...
    readonly SelectionElement ScenarioText = new SelectionElement();
    readonly SelectionElement ButtonResume = new SelectionElement();
    readonly SelectionElement ButtonMap = new SelectionElement();
    readonly SelectionElement ButtonMsgLog = new SelectionElement();
    readonly SelectionElement ButtonOptions = new SelectionElement();
    readonly SelectionElement ButtonQuit = new SelectionElement();

    public PauseMenu(Screen2D owner) : base(owner)
    {
      Engine.SoundManager.PauseMusic();
      Engine.SoundManager.StopAllSounds();
      Engine.SoundManager.StopSpeech();
    ...
    private bool SelectMap(CONST_TV_KEY key)
    {
      if (key == CONST_TV_KEY.TV_KEY_RETURN)
      {
        EnterPage(new ScenarioMap(Owner));
        return true;
      }
      return false;
    }
