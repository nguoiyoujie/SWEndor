...
namespace SWEndor.Game.UI.Menu.Pages
{
  public class KeyboardControls : Page
  {
    ...
    private bool Select(SelectionElement sel, CONST_TV_KEY key)
    {
      ...
      {
        _bindvals[sel] = key;
        sel.SecondaryText = Enum<CONST_TV_KEY>.GetName(key)?.Replace("TV_KEY_", "") ?? "";
        return true;
      }
      return false;
    }
    ...
    private void SaveBindings()
    {
      ...
      {
        int index = -1;
        while ((index = InputFunction.Registry.GetNext(kvp.Value, ++index, out InputFunction fn)) != -1)
        {
          fn.Key = (int)_bindvals[kvp.Key];
        }
      }
      LoadBindings();
    }
    ...
    private void LoadBindings()
    {
      foreach (var kvp in _bindings)
      {
        SelectionElement e = kvp.Key;
        int index = -1;
        while (InputFunction.Registry.GetNext(kvp.Value, ++index, out InputFunction fn) != -1)
        {
          _bindvals[e] = (CONST_TV_KEY)fn.Key;
          break;
        }

        e.SecondaryText = Enum<CONST_TV_KEY>.GetName(_bindvals[e])?.Replace("TV_KEY_", "") ?? "";
      }
    }
  }
}
