  ...
  internal class AttackActor : ActionInfo
  {
    ...
    private static TV_3DVECTOR MakeAltPosition(Engine engine, ActorInfo actor, ActorInfo target)
    {
      ...
      if (target != null)
      {
        ...
        center = sph.Position;
        float radius = sph.R + engine.Random.Next((int)(-4 * actor.MoveData.MaxSpeed), (int)(4 * actor.MoveData.MaxSpeed));
    ...
    private static bool AggressiveTrackingInner(Engine e, ActorInfo a, ActorInfo c)
    {
      if (a != null
            && c.ID != a.ID
            && a.Active
