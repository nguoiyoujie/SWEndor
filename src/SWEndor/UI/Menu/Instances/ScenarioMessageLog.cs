...
namespace SWEndor.UI.Menu.Pages
{
  public class ScenarioMessageLog : Page
  {
    ...
    List<SelectionElement> LogText = new List<SelectionElement>();
    ...
    public override void RenderTick()
    {
      ...
      {
        List<ScenarioStates.MessageLog> loglines = SelectedScenario.State.MessageLogs;
        if (i + log < loglines.Count)
        {
          LogText[i].Text = "{0:mm\\:ss}".F(TimeSpan.FromSeconds(loglines[i + log].Time));
