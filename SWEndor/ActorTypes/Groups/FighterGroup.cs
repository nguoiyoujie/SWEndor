    ...
    public override void ProcessState(ActorInfo ainfo)
    {
      ...
      if (ainfo.CreationState == CreationState.ACTIVE && (!GameScenarioManager.Instance().IsCutsceneMode || !ainfo.IsPlayer()))
      {
        ...
        if (!ainfo.IsPlayer())
        {
          if (dist < 200)
          {
            if (ainfo.TypeInfo is LandoFalconATI || ainfo.TypeInfo is FalconATI)
            {
              if (PlayerInfo.Instance().enginefcvol < 1 - dist / 200.0f)
                PlayerInfo.Instance().enginefcvol = 1 - dist / 200.0f;
            }
            else
            {
              if (PlayerInfo.Instance().enginesmvol < 1 - dist / 200.0f)
                PlayerInfo.Instance().enginesmvol = 1 - dist / 200.0f;
            }
          }
        }
      }
