...
namespace SWEndor.Evaluator
{
  public class Functions : Dictionary<string, Function>
  {
    ...
    public void InitDefaults()
    {
      ...
      this.Add("ifelse", new StaticFunction("IfElse", delegate (object[] ps) { return Convert.ToBoolean(ps[0]) ? ps[1] : ps[2]; }, 3, 3));
    ...
    {
      List<object> ls = new List<object>(ps);
      ls.Sort();
      object[] ordered = ls.ToArray();
    ...
    private static object Help(object[] ps)
    {
      ...
      List<string> ls = new List<string>(Defaults.Keys);
      ls.Sort();
      string[] keys = ls.ToArray();
