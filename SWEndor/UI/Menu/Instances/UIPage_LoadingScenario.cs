...
{
  public class UIPage_LoadingScenario : UIPage
  {
    UISelectionElement Cover = new UISelectionElement();
    UISelectionElement LoadingText = new UISelectionElement();
    ThreadSafeDictionary<UISelectionElement, float> Squares = new ThreadSafeDictionary<UISelectionElement, float>();
    ...
    public UIPage_LoadingScenario(GameScenarioBase selectedScenario, ActorTypeInfo selectedActorTypeInfo, string selectedDifficulty)
    {
      ...
      for (float i = 0.2f; i < 0.8f; i += di)
        ...
        {
          UISelectionElement sqi = new UISelectionElement();
      ...
      foreach (UISelectionElement sq in Squares.GetKeys())
        Elements.Add(sq);
    ...
    public override void Tick()
    {
      ...
      if (Loaded)
        ...
        Screen2D.Instance().CurrentPage = new UIPage_FatalError(LoadException);
      else
      {
        LoadingText.Text = PrintLoadingText();
