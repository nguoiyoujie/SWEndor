  ...
  public class AIInfo : Widget
  {
    ...
    public override bool Visible
    {
      get
      {
        return (!Owner.ShowPage
            ...
            && Owner.ShowUI
            && Owner.Engine.PlayerInfo.PlayerAIEnabled);
      }
    }
    ...
    public override void Draw()
    {
      ...
      if (p.CurrentAction != null)
      {
        ...
        {
          ActorInfo target = Owner.Engine.ActorFactory.Get(((AttackActor)p.CurrentAction).Target_ActorID);
          if (target != null)
          {
      ...
      TVScreen2DImmediate.Draw_FilledBox(Owner.Engine.ScreenWidth / 2 - 80, Owner.Engine.ScreenHeight / 2 - 25, Owner.Engine.ScreenWidth / 2 - 40, Owner.Engine.ScreenHeight / 2 + 5, new TV_COLOR(0, 0, 0, 0.5f).GetIntColor());
      TVScreen2DImmediate.Draw_FilledBox(Owner.Engine.ScreenWidth / 2 + 45, Owner.Engine.ScreenHeight / 2 - 25, Owner.Engine.ScreenWidth / 2 + 55 + 10 * name.Length, Owner.Engine.ScreenHeight / 2 + 5, new TV_COLOR(0, 0, 0, 0.5f).GetIntColor());
      ...
      TVScreen2DText.TextureFont_DrawText("AI"
        , Owner.Engine.ScreenWidth / 2 - 75, Owner.Engine.ScreenHeight / 2 - 20, new TV_COLOR(1, 0.5f, 0.2f, 1).GetIntColor(), Font.Factory.Get("Text_14").ID);

      TVScreen2DText.TextureFont_DrawText(name
        , Owner.Engine.ScreenWidth / 2 + 50, Owner.Engine.ScreenHeight / 2 - 20, new TV_COLOR(1, 0.5f, 0.2f, 1).GetIntColor(), Font.Factory.Get("Text_14").ID);
