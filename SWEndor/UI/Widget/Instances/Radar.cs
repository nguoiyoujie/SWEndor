...
namespace SWEndor.UI.Widgets
{
  public class Radar : Widget
  {
    ...
    private void DrawTargetingRadar(ActorInfo p)
    {
      ...
      float timefactor = Owner.Engine.Game.GameTime % 1;
      float divisor = 1.75f;
      ...
      foreach (int actorID in Owner.Engine.ActorFactory.GetHoldingList())
      {
        ...
        if (a != null)
        {
            ...
            && (a.TypeInfo.AlwaysShowInRadar || ActorDistanceInfo.GetRoughDistance(new TV_3DVECTOR(ppos.x, 0, ppos.z), new TV_3DVECTOR(apos.x, 0, apos.z)) < radar_range * 2))
          {
            ...
            float dist = Owner.Engine.TrueVision.TVMathLibrary.GetDistanceVec2D(new TV_2DVECTOR(), posvec);
            float angl = Owner.Engine.TrueVision.TVMathLibrary.Direction2Ang(posvec.x, posvec.y) - proty;
            if (dist > radar_range)
              dist = radar_range;
            ...
            {
              float gt = Owner.Engine.Game.GameTime * radar_blinkfreq;
              if (a.CombatInfo.Strength / a.TypeInfo.MaxStrength > 0.1f || (gt - (int)gt > 0.5f))
              {
                ...
                while (i < 4)
                {
                  ...
                  float tdist = Owner.Engine.TrueVision.TVMathLibrary.GetDistanceVec2D(new TV_2DVECTOR(), temp);
                  float tangl = Owner.Engine.TrueVision.TVMathLibrary.Direction2Ang(temp.x, temp.y) - proty;
            ...
            else if (a.TypeInfo is ActorTypes.Group.Projectile)
            {
              ...
              float prevdist = Owner.Engine.TrueVision.TVMathLibrary.GetDistanceVec2D(new TV_2DVECTOR(), prevtemp);
              float prevangl = Owner.Engine.TrueVision.TVMathLibrary.Direction2Ang(prevtemp.x, prevtemp.y) - p.GetRotation().y;
